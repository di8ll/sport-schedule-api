      ...
      <div>
        ...
        >
          <div className="absolute top-5 left-6 right-6 flex items-center gap-4 z-10 w-full max-w-7xl mx-auto">
            <button
              onClick={() => navigate("/")}
              className="flex items-center gap-1 text-xs font-medium bg-white/10 hover:bg-white/20 px-3 py-1.5 backdrop-blur rounded-md border border-white/20 transition-all shrink-0"
            >
              Imbangi ← Portal Utama
            </button>
            <h3 className="text-sm sm:text-base font-semibold tracking-wide text-white/90 drop-shadow-sm truncate">
              Indorama Founder's Day 2026
            </h3>
          </div>

          <div className="z-10 ml-auto mr-4 sm:mr-12 flex flex-col items-center text-center mt-6">
            <div className="flex items-center gap-2">
              <h2 className="text-4xl sm:text-6xl font-black tracking-tight uppercase italic leading-none drop-shadow-md">
                {theme.name}
              </h2>
        ...
        <div className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm backdrop-blur-md bg-white/95">
          <div className="max-w-7xl mx-auto flex justify-center gap-4 sm:gap-8 px-4">
            {[
              ...
              return (
                <button
                  ...
                  className={`py-4 px-3 text-xs sm:text-sm font-bold transition-all uppercase tracking-wider relative shrink-0 border-b-2 -mb-[2px] flex items-center gap-1.5 ${
                    isSelected
                  ...
                  {tab.id === "live" && liveCount > 0 && (
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono font-bold leading-none ${
                        isSelected ? "bg-red-500 text-white animate-pulse" : "bg-red-100 text-red-600"
                      }`}
                    >
                      {liveCount}
                    </span>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="text-base sm:text-lg font-black text-slate-800 tracking-wide uppercase">
                    Jadwal Hari: <span className="text-teal-700">{activeDate || "-"}</span>
                  </h3>
                </div>
                <span className="text-xs font-bold px-3 py-1 bg-teal-50 text-teal-700 border border-teal-200 rounded-full w-fit">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentMatchesToShow.map((match) => {
                  ...
                  return (
                    ...
                    >
                      {/* Top Header Card */}
                      <div className={`flex items-center justify-between px-4 sm:px-5 py-3 border-b-2 ${headerStyle}`}>
                        <div className="flex items-center gap-2 text-slate-600 min-w-0">
                          <span className={`text-xs font-bold font-mono px-2 py-0.5 rounded border shrink-0 ${badgeTimeStyle}`}>
                            {match.time}
                          </span>
                        ...
                        <div className="shrink-0">
                          {isLive ? (
                            <span className="inline-flex items-center gap-1 bg-red-600 text-white font-extrabold text-[10px] tracking-widest px-2.5 py-0.5 rounded-full uppercase border border-red-700 shadow-sm">
                              <span className="w-1.5 h-1.5 rounded-full bg-white animate-ping" />
                              LIVE
                            </span>
                          ) : isFinished ? (
                            <span className="bg-blue-600 text-white font-extrabold text-[10px] tracking-widest px-2.5 py-0.5 rounded-full uppercase border border-blue-700">
                              FINISHED
                            </span>
                          ) : (
                            <span className="bg-slate-200 text-slate-800 font-extrabold text-[10px] tracking-widest px-2.5 py-0.5 rounded-full uppercase border border-slate-300">
                              UPCOMING
                            </span>
                          )}
                        </div>
                      ...
                      {/* Main Match Info */}
                      <div className="p-4 sm:p-5 flex flex-col items-center justify-center bg-gradient-to-b from-white to-slate-50/50">
                        ...
                        <div className="w-full flex items-center justify-between gap-1.5">
                          ...
                          <div className="w-[24%] flex flex-col items-center justify-center">
                            {isUpcoming ? (
                              <div className="font-black italic text-xs tracking-wider text-slate-500 bg-slate-100 border-2 border-slate-300 px-2.5 py-1 rounded-xl shadow-sm">
                                VS
                              </div>
                            ) : (
                              <div className="flex items-center justify-center gap-0.5 sm:gap-1">
                                <span
                                  className={`text-lg sm:text-2xl font-mono font-black px-2 py-0.5 rounded-xl border-2 ${
                                    isLive ? "bg-red-50 border-red-500 text-red-600" : "bg-blue-50 border-blue-500 text-blue-600"
                                  }`}
                                >
                                  {match.scoreA}
                                </span>
                        ...
                        <div className="w-full text-center mt-3 pt-2 border-t border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-200">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                    <button
                      ...
                      className="flex-1 sm:flex-initial text-xs font-bold uppercase tracking-wider px-4 py-2.5 rounded-xl border-2 bg-white transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 active:scale-95 text-slate-700 border-slate-300"
                    >
                      ← Halaman Sebelumnya
                    </button>
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-initial text-xs font-bold uppercase tracking-wider px-4 py-2.5 rounded-xl border-2 bg-teal-700 border-teal-800 text-white transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-teal-800 active:scale-95 shadow-sm"
                    >
                      Halaman Berikutnya →
                    </button>
                  </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-5 p-5">
              ...
              <div className="flex items-center justify-between bg-slate-50 rounded-2xl p-4 border border-slate-100">
                ...
                <div className="w-[30%] flex flex-col items-center">
                  ...
                  <span
                    className={`text-[9px] font-extrabold tracking-widest mt-1 px-2 py-0.5 rounded-full uppercase ${
                      selectedMatch.status === "LIVE" ? "bg-red-100 text-red-600 animate-pulse" : "bg-blue-100 text-blue-600"
                    }`}
                  >
